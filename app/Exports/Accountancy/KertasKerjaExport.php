...
class KertasKerjaExport implements FromCollection, WithHeadings, WithColumnFormatting, WithStyles, WithColumnWidths
{
    ...
    public function headings(): array
    {
        if ($this->params['category'] == 'kibs') {
            if ($this->params['type'] == 'rekon_aset_penyusutan') {
                return [
                    'Nama Aset',
                    'Akumulasi Penyusutan Tahun Lalu',
                    'Mutasi Tambah',
                    'Mutasi Kurang',
                    'Akumulasi Penyusutan Tahun Ini',
                ];
            } else {
                return [
                    'Perangkat Daerah',
        ...
        if ($this->params['category'] == 'kibs') {
            if ($this->params['type'] == 'rekon_aset_penyusutan') {
                ...
                foreach ($collectDatas as $data) {
                    $datas[] = [
                        $data['uraian'],
                        $data['akumulasi_penyusutan_last_year'],
                        $data['mutasi_tambah'],
                        $data['mutasi_kurang'],
                ...
                $datas[] = [
                    'Total',
                    $collectDatas->sum('akumulasi_penyusutan_last_year'),
                    $collectDatas->sum('mutasi_tambah'),
                    $collectDatas->sum('mutasi_kurang'),
