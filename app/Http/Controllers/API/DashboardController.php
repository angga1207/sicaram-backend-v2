...
class DashboardController extends Controller
{
    ...
    function chartRealisasi(Request $request)
    {
        ...
        foreach ($rangeMonths as $month) {
            ...
            $paguPerubahan = $instanceSummary
                ->sum('pagu_perubahan');
            $paguPergeseran4 = $instanceSummary
                ->sum('pagu_pergeseran_4');
            $paguPergeseran3 = $instanceSummary
                ->sum('pagu_pergeseran_3');
            $paguPergeseran2 = $instanceSummary
                ->sum('pagu_pergeseran_2');
            $paguPergeseran1 = $instanceSummary
                ->sum('pagu_pergeseran_1');
            $paguAnggaran = $instanceSummary
                ->sum('pagu_anggaran');

            $notes = '';

            if ($paguPerubahan > 0) {
                $sumTarget = $paguPerubahan;
                $notes = 'Pagu Perubahan';
            } else if ($paguPergeseran4 > 0) {
                $sumTarget = $paguPergeseran4;
                $notes = 'Pagu Pergeseran 4';
            } else if ($paguPergeseran3 > 0) {
                $sumTarget = $paguPergeseran3;
                $notes = 'Pagu Pergeseran 3';
            } else if ($paguPergeseran2 > 0) {
                $sumTarget = $paguPergeseran2;
                $notes = 'Pagu Pergeseran 2';
            } else if ($paguPergeseran1 > 0) {
                $sumTarget = $paguPergeseran1;
                $notes = 'Pagu Pergeseran 1';
            } else {
                $sumTarget = $paguAnggaran;
                $notes = 'Pagu Induk';
            }
